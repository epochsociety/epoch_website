import { a5 as useRouter, T as reactExports, K as jsxRuntimeExports } from "./server-vE8pAmWh.js";
import { M as MotionConfigContext, i as isHTMLElement, u as useConstant, P as PresenceContext, f as usePresence, d as useIsomorphicLayoutEffect, L as LayoutGroupContext, m as motion, a as Link, e as useNavigate } from "./router-BBmLPuFF.js";
...
function useLocation(opts) {
  ...
  {
    const location2 = router.stores.location.get();
    return location2;
  }
}
...
class GoTrueAdminApi {
  ...
  async listUsers(params) {
    ...
    try {
      ...
      const links2 = (_g = (_f = response.headers.get("link")) === null || _f === void 0 ? void 0 : _f.split(",")) !== null && _g !== void 0 ? _g : [];
      if (links2.length > 0) {
        links2.forEach((link) => {
...
const supabaseUrl = "https://csjsprbxyvehbqdkxyfn.supabase.co";
const supabaseAnonKey = "sb_publishable_42GjBDd8YVV8xK4bq-vOTw_ISwclrjq";
const isPlaceholder = supabaseUrl.includes("your-project-id") || supabaseAnonKey.includes("your-supabase-anon-key");
const supabase = !isPlaceholder ? createClient(supabaseUrl, supabaseAnonKey) : null;
if (!supabase) {
  if (isPlaceholder) {
    console.warn(
      "Supabase: VITE_SUPABASE_URL is configured with placeholder values. Please update your .env file with your actual credentials. Falling back to LocalStorage."
    );
  } else {
    console.warn(
      "Supabase: VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY are not defined. Falling back to LocalStorage."
    );
  }
}
...
if (supabase && typeof window !== "undefined") {
  const syncCreds = async () => {
    try {
      const { data: credsData, error: credsErr } = await supabase.from("epoch_creds").select("*");
      if (credsErr) console.error("Supabase load creds error:", credsErr);
      if (!credsData || credsData.length === 0) {
        const localCreds = getCreds();
        for (const role of ["developer", "faculty"]) {
          await supabase.from("epoch_creds").upsert({
            role,
            username: localCreds[role].username,
            password: localCreds[role].password
          });
        }
      } else {
        const localCreds = getCreds();
        for (const row of credsData) {
          if (row.role === "developer" || row.role === "faculty") {
            localCreds[row.role] = {
        ...
        localStorage.setItem(CREDS_KEY, JSON.stringify(localCreds));
      }
    } catch (e) {
      console.error("Failed to sync credentials with Supabase:", e);
    }
  };
  syncCreds();
  supabase.channel("creds-db-changes").on(
    "postgres_changes",
    { event: "*", schema: "public", table: "epoch_creds" },
    syncCreds
  ).subscribe();
}
function updatePassword(role, newPassword) {
  ...
  if (supabase) {
    supabase.from("epoch_creds").upsert({
      role,
      username: creds[role].username,
      password: newPassword,
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    }).then(({ error }) => {
      if (error) console.error("Supabase updatePassword error:", error);
    });
  }
}
            ...
            children: /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "flex items-center justify-between gap-2", children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("ul", { className: "flex items-center gap-1", children: links.map((l) => /* @__PURE__ */ jsxRuntimeExports.jsx("li", { children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                Link,
                {
                  ...
                  children: ({ isActive }) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    motion.div,
                    {
                      whileTap: { scale: 0.95 },
              ...
              ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center gap-1 pr-1", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                Link,
                {
                  to: "/login",
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("footer", { className: "relative mt-32 pb-10 pt-16 px-6", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "max-w-6xl mx-auto grid gap-10 md:grid-cols-3 items-start", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-foreground", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/login", className: "hover:text-foreground transition", children: "Staff Login" })
      ] }),
...
let dbCache = readLocalStorage();
if (supabase && typeof window !== "undefined") {
  const loadData = async () => {
    try {
      const { data: contentData, error: contentErr } = await supabase.from("epoch_content").select("data").eq("id", "global").maybeSingle();
      if (contentErr) console.error("Supabase load content error:", contentErr);
      let finalContent = contentData?.data;
      if (!contentData) {
        await supabase.from("epoch_content").upsert({ id: "global", data: seed.content });
        finalContent = seed.content;
      }
      const { data: membersData, error: membersErr } = await supabase.from("epoch_members").select("*").order("created_at", { ascending: true });
      if (membersErr) console.error("Supabase load members error:", membersErr);
      let finalMembers = membersData;
      if (!membersData || membersData.length === 0) {
        for (const m of seed.members) {
          await supabase.from("epoch_members").upsert(m);
        }
        finalMembers = seed.members;
      }
      const { data: eventsData, error: eventsErr } = await supabase.from("epoch_events").select("*").order("created_at", { ascending: true });
      if (eventsErr) console.error("Supabase load events error:", eventsErr);
      let finalEvents = eventsData;
      if (!eventsData || eventsData.length === 0) {
        for (const e of seed.events) {
          await supabase.from("epoch_events").upsert(e);
        }
        finalEvents = seed.events;
      }
      dbCache = {
        content: finalContent || seed.content,
        members: (finalMembers || []).map((m) => ({
          ...
          image: m.image,
          bio: m.bio,
          linkedin: m.linkedin || void 0,
          year: m.year || void 0,
          email: m.email || void 0,
          phone: m.phone || void 0
        })),
        events: (finalEvents || []).map((e) => ({
          ...
          registerUrl: e.registerUrl || void 0,
          isUpcoming: e.isUpcoming ?? true
        }))
      };
      writeLocalStorage(dbCache);
    } catch (e) {
  ...
  supabase.channel("schema-db-changes").on(
    "postgres_changes",
    { event: "*", schema: "public", table: "epoch_content" },
    loadData
  ).on(
    "postgres_changes",
    { event: "*", schema: "public", table: "epoch_members" },
    loadData
  ).on(
    "postgres_changes",
    { event: "*", schema: "public", table: "epoch_events" },
    loadData
  ).subscribe();
}
const store = {
  getAll: () => dbCache,
  getContent: () => dbCache.content,
  setContent: (c) => {
    dbCache.content = { ...dbCache.content, ...c };
    writeLocalStorage(dbCache);
    if (supabase) {
      supabase.from("epoch_content").upsert({ id: "global", data: dbCache.content }).then(({ error }) => {
        if (error) console.error("Supabase setContent error:", error);
      });
    }
  },
  getMembers: () => dbCache.members,
  upsertMember: (m) => {
    const idx = dbCache.members.findIndex((x) => x.id === m.id);
    if (idx >= 0) dbCache.members[idx] = m;
    else dbCache.members.push(m);
    writeLocalStorage(dbCache);
    if (supabase) {
      supabase.from("epoch_members").upsert(m).then(({ error }) => {
        if (error) console.error("Supabase upsertMember error:", error);
      });
    }
  },
  removeMember: (id) => {
    dbCache.members = dbCache.members.filter((m) => m.id !== id);
    writeLocalStorage(dbCache);
    if (supabase) {
      supabase.from("epoch_members").delete().eq("id", id).then(({ error }) => {
        if (error) console.error("Supabase removeMember error:", error);
      });
    }
  },
  getEvents: () => dbCache.events,
  getEvent: (id) => dbCache.events.find((e) => e.id === id),
  upsertEvent: (e) => {
    const idx = dbCache.events.findIndex((x) => x.id === e.id);
    if (idx >= 0) dbCache.events[idx] = e;
    else dbCache.events.push(e);
    writeLocalStorage(dbCache);
    if (supabase) {
      supabase.from("epoch_events").upsert({
        id: e.id,
        title: e.title,
        date: e.date,
        description: e.description,
        ...
        if (error) console.error("Supabase upsertEvent error:", error);
      });
    }
  },
  addDocument: (eventId, doc) => {
    const e = dbCache.events.find((x) => x.id === eventId);
    if (!e) return;
    e.documents.push(doc);
    writeLocalStorage(dbCache);
    if (supabase) {
      supabase.from("epoch_events").update({ documents: e.documents }).eq("id", eventId).then(({ error }) => {
  ...
  removeDocument: (eventId, docId) => {
    ...
    if (supabase) {
      supabase.from("epoch_events").update({ documents: e.documents }).eq("id", eventId).then(({ error }) => {
        if (error) console.error("Supabase removeDocument error:", error);
      });
    }
  },
  removeEvent: (id) => {
    ...
    writeLocalStorage(dbCache);
    if (supabase) {
      supabase.from("epoch_events").delete().eq("id", id).then(({ error }) => {
        if (error) console.error("Supabase removeEvent error:", error);
      });
    }
  }
};
